...
| Исполнитель задачи | `t797` | ID исполнителя или список через запятую |
| Исполнитель операции | `t2667` | ID исполнителя или список через запятую |

### 3.2 Происхождение кода `t2667`

**Код `t2667` (operationExecutorCode)** был введён в рамках решения **Issue #3** ("Написать JS для назначения исполнителей на задачи и операции проекта") и реализован в **Pull Request #4**.

**Контекст и обоснование:**
- В системе integram.io каждое поле имеет уникальный код, начинающийся с `t` (типа данных)
- Код `t2667` представляет поле "Исполнитель операции" в базе данных
- Этот код используется для сохранения назначенных исполнителей на операции через API
- Аналогичный код для задач - `t797` (исполнитель задачи)
...
3. PR #4 - реализован полный алгоритм назначения исполнителей с сохранением в поля `t797` (задачи) и `t2667` (операции)
4. Issues #5, #7, #9, #11, #13, #15 - улучшения логики параметров и их объединения

**Техническое использование:**
```javascript
const CONFIG = {
    taskExecutorCode: 't797',        // Код поля исполнителя задачи
    operationExecutorCode: 't2667',  // Код поля исполнителя операции
};
...
const CONFIG = {
    ...
    taskExecutorCode: 't797',        // Код параметра исполнителя задачи
    operationExecutorCode: 't2667',  // Код параметра исполнителя операции
...
- Описано происхождение кода `t2667`
...
- **Pull Request #4:** https://github.com/ideav/orbits/pull/4 - реализация назначения исполнителей (происхождение t2667)
...
- Все коды полей (включая `t2667`) определены в соответствии со структурой базы данных integram.io
